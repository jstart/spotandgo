...
# :tag => CATEGORY[params['category'].to_sym][:tag]
	response = CityGridApi::LatLonSearch.find({:what => CATEGORY[params['category'].to_sym][:tag_name], :lat => params["location"][0], :lon => params["location"][1], :rpp => 4, :page => 1, :sort => "highestrated", :radius => 2})
